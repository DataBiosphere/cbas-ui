...
describe('Add a Workflow Link', () => {
  beforeEach(() => {
    getConfig.mockReturnValue({ isURLEnabled: true })
    jest.clearAllMocks()
  })
  ...
  it('should render workflow link subheader with config enabled', async () => {
    ...
    expect(screen.getByText('Add a Workflow Link')).toBeInTheDocument()
  })
  ...
  it('should render text inputs/headers', async () => {
    ...
    const workflowLinkButton = screen.getByText('Add a Workflow Link')
  ...
  it('should fail when given a non github link', async () => {
    ...
    // ** ACT **
    render(h(FindWorkflowModal, { onDismiss }))
